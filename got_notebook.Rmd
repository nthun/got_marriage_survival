...
In the next block, we transform the data from the awkard hierarchical data structure into a data frame, that is easy to use. We also clean the data to get meaningful variables. 
...
We restrict the analysis to characters that were born within a 102 years (This is the age of oldest character that dies in the books) because we want to focus on the book's storyline. We are only looking for main effects of age, gender, and marital status (if the character has a spouse). 
...
To explore data, we create a boxplot visualization about age by gender by marital status by survival status.
...
- We looked at dying in the last 102 years, so we excluded characters who died long before the storyline 
- Age is not a significant predictor for dying, so people of all ages perish left and right
- Males have a worse survival chance (they are  twice as likely to die in the books' storyline)
